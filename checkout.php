...
// ... aapka purana cart code ...

if(isset($_POST['placeorder'])) {
    // 1. Check karein user login hai ya nahi
    if (!isset($_SESSION['user_id'])) {
        echo "<script>alert('Please login first to place order!'); window.location='login.php';</script>";
        exit();
    }
    ...
    // 2. Pehle 'orders' table mein main order entry karein
    $order_query = "INSERT INTO orders (user_id, total_amount) VALUES ('$user_id', '$final_total')";
    
    if (mysqli_query($conn, $order_query)) {
        // Naye order ki ID nikalein
        $order_id = mysqli_insert_id($conn);

        // 3. Ab Cart ke har item ko 'order_items' table mein insert karein
        foreach ($_SESSION['cart'] as $p_id => $details) {
            $p_name = $details['name'];
            $p_price = $details['price'];
            $p_qty = $details['quantity'];

            $item_query = "INSERT INTO order_items (order_id, product_id, product_name, product_price, product_qty) 
                           VALUES ('$order_id', '$p_id', '$p_name', '$p_price', '$p_qty')";
            mysqli_query($conn, $item_query);
        }

        // 4. Order hone ke baad Cart khaali karein
        unset($_SESSION['cart']);
...
<body>
    ...
    <nav class="navbar navbar-expand-lg bg-white shadow-sm mb-4 sticky-top">
        <div class="container-fluid">
            ...
            <div class="d-flex gap-4 fs-4 align-items-center">
                ...
                <?php else: ?>
                    <a href="login.php" class="text-dark fs-6 fw-bold text-decoration-none"><i class="fa-solid fa-user"></i>
                        Login</a>
                <?php endif; ?>
    ...
    <div class="price-details-card">
        ...
        <form method="POST">
            ...
            <button type="submit" name="placeorder" class="btn-continue">
                Place Order
            </button>
        </form>
    </div>
